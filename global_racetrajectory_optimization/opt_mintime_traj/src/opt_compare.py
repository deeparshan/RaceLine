...
def opt_compare(reftrack1: np.ndarray,
                coeffs_x1: np.ndarray,
                coeffs_y1: np.ndarray,
                normvectors1: np.ndarray,
                pars1: dict,
                tpamap_path1: str,
                tpadata_path1: str,
                export_path1: str,
                reftrack2: np.ndarray,
                coeffs_x2: np.ndarray,
                coeffs_y2: np.ndarray,
                normvectors2: np.ndarray,
                pars2: dict,
                tpamap_path2: str,
                tpadata_path2: str,
                export_path2: str,
    ...
    no_points_orig = reftrack1.shape[0]

    if pars1["optim_opts"]["step_non_reg"] > 0:
        reftrack, discr_points = tph.nonreg_sampling.nonreg_sampling(track=reftrack1,
                                                                     eps_kappa=pars1["optim_opts"]["eps_kappa"],
                                                                     step_non_reg=pars1["optim_opts"]["step_non_reg"])
        ...
        discr_points = np.arange(reftrack1.shape[0])
        a_interp = None
        coeffs_x = coeffs_x1
        coeffs_y = coeffs_y1
        reftrack = reftrack1
        normvectors = normvectors1
    ...
    kappa_refline = tph.calc_head_curv_num. \
        calc_head_curv_num(path=reftrack[:, :2],
                           ...
                           is_closed=True,
                           stepsize_curv_preview=pars1["curv_calc_opts"]["d_preview_curv"],
                           stepsize_curv_review=pars1["curv_calc_opts"]["d_review_curv"],
                           stepsize_psi_preview=pars1["curv_calc_opts"]["d_preview_head"],
                           stepsize_psi_review=pars1["curv_calc_opts"]["d_review_head"])[1]
    ...
    h = pars1["stepsize_opts"]["stepsize_reg"]
    ...
    if pars1["optim_opts"]["var_friction"] is not None:
        w_mue_fl, w_mue_fr, w_mue_rl, w_mue_rr, center_dist = opt_mintime_traj.src. \
            approx_friction_map.approx_friction_map(reftrack=reftrack,
                                                    ...
                                                    tpamap_path=tpamap_path1,
                                                    tpadata_path=tpadata_path1,
                                                    pars=pars1,
                                                    dn=pars1["optim_opts"]["dn"],
                                                    n_gauss=pars1["optim_opts"]["n_gauss"],
    ...
    if pars1["pwr_params_mintime"]["pwr_behavior"]:
        ...
        machine = opt_mintime_traj.powertrain_src.src.EMachine.EMachineModel(pwr_pars=pars1["pwr_params_mintime"])

        # Initialize battery object
        batt = opt_mintime_traj.powertrain_src.src.Battery.BattModel(pwr_pars=pars1["pwr_params_mintime"])

        # Initialize inverter object
        inverter = opt_mintime_traj.powertrain_src.src.Inverter.InverterModel(pwr_pars=pars1["pwr_params_mintime"])

        # Initialize radiator objects (2 in total)
        radiators = opt_mintime_traj.powertrain_src.src.Radiators.RadiatorModel(pwr_pars=pars1["pwr_params_mintime"])
    ...
    veh = pars1["vehicle_params_mintime"]
    tire = pars1["tire_params_mintime"]

    # general constants
    g = pars1["veh_params"]["g"]
    mass = pars1["veh_params"]["mass"]

    # curvature of reference line [rad/m]
    kappa = ca.SX.sym('kappa')

    # drag force [N]
    f_xdrag = pars1["veh_params"]["dragcoeff"] * v ** 2
    ...
    ax = (f_x_rl + f_x_rr + (f_x_fl + f_x_fr) * ca.cos(delta) - (f_y_fl + f_y_fr) * ca.sin(delta)
          - pars1["veh_params"]["dragcoeff"] * v ** 2) / mass
    ...
    if pars1["pwr_params_mintime"]["pwr_behavior"]:
        ...
        pwr_pars = pars1["pwr_params_mintime"]
    ...
    v_max = pars1["veh_params"]["v_max"] / v_s       # max. velocity [m/s]
    ...
    Xk = ca.MX.sym('X0', nx)
    w.append(Xk)
    n_min = (-w_tr_right_interp(0) + pars1["optim_opts"]["width_opt"] / 2) / n_s
    n_max = (w_tr_left_interp(0) - pars1["optim_opts"]["width_opt"] / 2) / n_s
    if pars1["pwr_params_mintime"]["pwr_behavior"]:
    ...
    for k in range(N):
        ...
        w.append(Xk)
        n_min = (-w_tr_right_interp(k + 1) + pars1["optim_opts"]["width_opt"] / 2.0) / n_s
        n_max = (w_tr_left_interp(k + 1) - pars1["optim_opts"]["width_opt"] / 2.0) / n_s
        if pars1["pwr_params_mintime"]["pwr_behavior"]:
            lbw.append([v_min, beta_min, omega_z_min, n_min, xi_min,
                        machine.temp_min, batt.temp_min, inverter.temp_min,
                        radiators.temp_cool_mi_min, radiators.temp_cool_b_min,
        ...
        if pars1["optim_opts"]["var_friction"] is None:
            mue_fl = pars1["optim_opts"]["mue"]
            mue_fr = pars1["optim_opts"]["mue"]
            mue_rl = pars1["optim_opts"]["mue"]
            mue_rr = pars1["optim_opts"]["mue"]

        # calculate variable friction coefficients along the reference line (regression with linear equations)
        elif pars1["optim_opts"]["var_friction"] == "linear":
        ...
        elif pars1["optim_opts"]["var_friction"] == "gauss":
        ...
        if pars1["optim_opts"]["safe_traj"]:
            g.append((ca.fmax(axk, 0) / pars1["optim_opts"]["ax_pos_safe"]) ** 2
                     + (ayk / pars1["optim_opts"]["ay_safe"]) ** 2)
            g.append((ca.fmin(axk, 0) / pars1["optim_opts"]["ax_neg_safe"]) ** 2
                     + (ayk / pars1["optim_opts"]["ay_safe"]) ** 2)
            lbg.append([0.0] * 2)
            ubg.append([1.0] * 2)
    ...
    if pars1["optim_opts"]["limit_energy"]:
        g.append(ca.sum1(ca.vertcat(*ec_opt)) / 3600000.0)
        lbg.append([0])
        ubg.append([pars1["optim_opts"]["energy_limit"]])
    ...
    # formulate objective
    J = J + pars1["optim_opts"]["penalty_F"] * Jp_f + pars1["optim_opts"]["penalty_delta"] * Jp_delta
    ...
    if pars1["optim_opts"]["warm_start"]:
    ...
    if pars1["optim_opts"]["warm_start"]:
        try:
            w0 = np.loadtxt(os.path.join(export_path1, 'w0.csv'))
            lam_x0 = np.loadtxt(os.path.join(export_path1, 'lam_x0.csv'))
            lam_g0 = np.loadtxt(os.path.join(export_path1, 'lam_g0.csv'))
        except IOError:
            print('\033[91m' + 'WARNING: Failed to load warm start files!' + '\033[0m')
            sys.exit(1)

    # check warm start files
    if pars1["optim_opts"]["warm_start"] and not len(w0) == len(lbw):
    ...
    # export data to CSVs
    opt_mintime_traj.src.export_mintime_solution.export_mintime_solution(file_path=export_path1,
                                                                         pars=pars1,
                                                                         s=s_opt,
                                                                         t=t_opt,
                                                                         x=x_opt,
    ...
    no_points_orig = reftrack2.shape[0]

    if pars2["optim_opts"]["step_non_reg"] > 0:
        reftrack, discr_points = tph.nonreg_sampling.nonreg_sampling(track=reftrack2,
                                                                     eps_kappa=pars2["optim_opts"]["eps_kappa"],
                                                                     step_non_reg=pars2["optim_opts"]["step_non_reg"])
    ...
    kappa_refline = tph.calc_head_curv_num. \
        calc_head_curv_num(path=reftrack[:, :2],
                           el_lengths=spline_lengths_refline,
                           is_closed=True,
                           stepsize_curv_preview=pars2["curv_calc_opts"]["d_preview_curv"],
                           stepsize_curv_review=pars2["curv_calc_opts"]["d_review_curv"],
                           stepsize_psi_preview=pars2["curv_calc_opts"]["d_preview_head"],
                           stepsize_psi_review=pars2["curv_calc_opts"]["d_review_head"])[1]
    ...
    # step size along the reference line
    h = pars2["stepsize_opts"]["stepsize_reg"]
    ...
    # describe friction coefficients from friction map with linear equations or gaussian basis functions
    if pars2["optim_opts"]["var_friction"] is not None:
        w_mue_fl, w_mue_fr, w_mue_rl, w_mue_rr, center_dist = opt_mintime_traj.src. \
            approx_friction_map.approx_friction_map(reftrack=reftrack,
                                                    normvectors=normvectors,
                                                    tpamap_path=tpamap_path2,
                                                    tpadata_path=tpadata_path2,
                                                    pars=pars2,
                                                    dn=pars2["optim_opts"]["dn"],
                                                    n_gauss=pars2["optim_opts"]["n_gauss"],
                                                    print_debug=print_debug,
                                                    plot_debug=plot_debug)
    ...
    if pars2["pwr_params_mintime"]["pwr_behavior"]:
        ...
        # Initialize e-machine object
        machine = opt_mintime_traj.powertrain_src.src.EMachine.EMachineModel(pwr_pars=pars2["pwr_params_mintime"])

        # Initialize battery object
        batt = opt_mintime_traj.powertrain_src.src.Battery.BattModel(pwr_pars=pars2["pwr_params_mintime"])

        # Initialize inverter object
        inverter = opt_mintime_traj.powertrain_src.src.Inverter.InverterModel(pwr_pars=pars2["pwr_params_mintime"])

        # Initialize radiator objects (2 in total)
        radiators = opt_mintime_traj.powertrain_src.src.Radiators.RadiatorModel(pwr_pars=pars2["pwr_params_mintime"])
    ...
    # extract vehicle and tire parameters
    veh = pars2["vehicle_params_mintime"]
    tire = pars2["tire_params_mintime"]

    # general constants
    g = pars2["veh_params"]["g"]
    mass = pars2["veh_params"]["mass"]

    # curvature of reference line [rad/m]
    kappa = ca.SX.sym('kappa')

    # drag force [N]
    f_xdrag = pars2["veh_params"]["dragcoeff"] * v ** 2
    ...
    # lateral tire forces [N]
    f_y_fl = (pars2["optim_opts"]["mue"] * f_z_fl * (1 + tire["eps_front"] * f_z_fl / tire["f_z0"])
              * ca.sin(tire["C_front"] * ca.atan(tire["B_front"] * alpha_fl - tire["E_front"]
                                                 * (tire["B_front"] * alpha_fl - ca.atan(tire["B_front"] * alpha_fl)))))
    f_y_fr = (pars2["optim_opts"]["mue"] * f_z_fr * (1 + tire["eps_front"] * f_z_fr / tire["f_z0"])
              * ca.sin(tire["C_front"] * ca.atan(tire["B_front"] * alpha_fr - tire["E_front"]
                                                 * (tire["B_front"] * alpha_fr - ca.atan(tire["B_front"] * alpha_fr)))))
    f_y_rl = (pars2["optim_opts"]["mue"] * f_z_rl * (1 + tire["eps_rear"] * f_z_rl / tire["f_z0"])
              * ca.sin(tire["C_rear"] * ca.atan(tire["B_rear"] * alpha_rl - tire["E_rear"]
                                                * (tire["B_rear"] * alpha_rl - ca.atan(tire["B_rear"] * alpha_rl)))))
    f_y_rr = (pars2["optim_opts"]["mue"] * f_z_rr * (1 + tire["eps_rear"] * f_z_rr / tire["f_z0"])
              * ca.sin(tire["C_rear"] * ca.atan(tire["B_rear"] * alpha_rr - tire["E_rear"]
                                                * (tire["B_rear"] * alpha_rr - ca.atan(tire["B_rear"] * alpha_rr)))))
    ...
    # longitudinal acceleration [m/s²]
    ax = (f_x_rl + f_x_rr + (f_x_fl + f_x_fr) * ca.cos(delta) - (f_y_fl + f_y_fr) * ca.sin(delta)
          - pars2["veh_params"]["dragcoeff"] * v ** 2) / mass
    ...
    if pars2["pwr_params_mintime"]["pwr_behavior"]:
        ...
        pwr_pars = pars2["pwr_params_mintime"]
    ...
    v_min = 1.0 / v_s                               # min. velocity [m/s]
    v_max = pars2["veh_params"]["v_max"] / v_s       # max. velocity [m/s]
    beta_min = -0.5 * np.pi / beta_s                # min. side slip angle [rad]
    beta_max = 0.5 * np.pi / beta_s                 # max. side slip angle [rad]
    omega_z_min = - 0.5 * np.pi / omega_z_s         # min. yaw rate [rad/s]
    omega_z_max = 0.5 * np.pi / omega_z_s           # max. yaw rate [rad/s]
    ...
    Xk = ca.MX.sym('X0', nx)
    w.append(Xk)
    n_min = (-w_tr_right_interp(0) + pars2["optim_opts"]["width_opt"] / 2) / n_s
    n_max = (w_tr_left_interp(0) - pars2["optim_opts"]["width_opt"] / 2) / n_s
    if pars2["pwr_params_mintime"]["pwr_behavior"]:
        lbw.append([v_min, beta_min, omega_z_min, n_min, xi_min,
                    machine.temp_min, batt.temp_min, inverter.temp_min,
                    radiators.temp_cool_mi_min, radiators.temp_cool_b_min,
    ...
    for k in range(N):
        ...
        Xk = ca.MX.sym('X_' + str(k + 1), nx)
        w.append(Xk)
        n_min = (-w_tr_right_interp(k + 1) + pars2["optim_opts"]["width_opt"] / 2.0) / n_s
        n_max = (w_tr_left_interp(k + 1) - pars2["optim_opts"]["width_opt"] / 2.0) / n_s
        if pars2["pwr_params_mintime"]["pwr_behavior"]:
            lbw.append([v_min, beta_min, omega_z_min, n_min, xi_min,
                        machine.temp_min, batt.temp_min, inverter.temp_min,
                        radiators.temp_cool_mi_min, radiators.temp_cool_b_min,
        ...
        # get constant friction coefficient
        if pars2["optim_opts"]["var_friction"] is None:
            mue_fl = pars2["optim_opts"]["mue"]
            mue_fr = pars2["optim_opts"]["mue"]
            mue_rl = pars2["optim_opts"]["mue"]
            mue_rr = pars2["optim_opts"]["mue"]

        # calculate variable friction coefficients along the reference line (regression with linear equations)
        elif pars2["optim_opts"]["var_friction"] == "linear":
            # friction coefficient for each tire
            mue_fl = w_mue_fl[k + 1, 0] * Xk[3] * n_s + w_mue_fl[k + 1, 1]
            mue_fr = w_mue_fr[k + 1, 0] * Xk[3] * n_s + w_mue_fr[k + 1, 1]
            mue_rl = w_mue_rl[k + 1, 0] * Xk[3] * n_s + w_mue_rl[k + 1, 1]
        ...
        # calculate variable friction coefficients along the reference line (regression with gaussian basis functions)
        elif pars2["optim_opts"]["var_friction"] == "gauss":
            # gaussian basis functions
            sigma = 2.0 * center_dist[k + 1, 0]
            n_gauss = pars2["optim_opts"]["n_gauss"]
            n_q = np.linspace(-n_gauss, n_gauss, 2 * n_gauss + 1) * center_dist[k + 1, 0]
        ...
        # path constraint: safe trajectories with acceleration ellipse
        if pars2["optim_opts"]["safe_traj"]:
            g.append((ca.fmax(axk, 0) / pars2["optim_opts"]["ax_pos_safe"]) ** 2
                     + (ayk / pars2["optim_opts"]["ay_safe"]) ** 2)
            g.append((ca.fmin(axk, 0) / pars2["optim_opts"]["ax_neg_safe"]) ** 2
                     + (ayk / pars2["optim_opts"]["ay_safe"]) ** 2)
            lbg.append([0.0] * 2)
            ubg.append([1.0] * 2)
    ...
    # path constraint: limited energy consumption
    if pars2["optim_opts"]["limit_energy"]:
        g.append(ca.sum1(ca.vertcat(*ec_opt)) / 3600000.0)
        lbg.append([0])
        ubg.append([pars2["optim_opts"]["energy_limit"]])
    ...
    # formulate objective
    J = J + pars2["optim_opts"]["penalty_F"] * Jp_f + pars2["optim_opts"]["penalty_delta"] * Jp_delta
    ...
    # solver options for warm start
    if pars2["optim_opts"]["warm_start"]:
        opts_warm_start = {"ipopt.warm_start_init_point": "yes",
                           "ipopt.warm_start_bound_push": 1e-3,
                           "ipopt.warm_start_mult_bound_push": 1e-3,
                           "ipopt.warm_start_slack_bound_push": 1e-3,
    ...
    if pars2["optim_opts"]["warm_start"]:
        try:
            w0 = np.loadtxt(os.path.join(export_path2, 'w0.csv'))
            lam_x0 = np.loadtxt(os.path.join(export_path2, 'lam_x0.csv'))
            lam_g0 = np.loadtxt(os.path.join(export_path2, 'lam_g0.csv'))
        except IOError:
            print('\033[91m' + 'WARNING: Failed to load warm start files!' + '\033[0m')
            sys.exit(1)

    # check warm start files
    if pars2["optim_opts"]["warm_start"] and not len(w0) == len(lbw):
        print('\033[91m' + 'WARNING: Warm start files do not fit to the dimension of the NLP!' + '\033[0m')
        sys.exit(1)
    ...
    # export data to CSVs
    opt_mintime_traj.src.export_mintime_solution.export_mintime_solution(file_path=export_path2,
                                                                         pars=pars2,
                                                                         s=s_opt,
                                                                         t=t_opt,
                                                                         x=x_opt,
    ...
    ### PLOT
    if plot_debug:
#        opt_mintime_traj.src.result_plots_mintime.result_plots_mintime(pars=pars2,
#                                                                       reftrack=reftrack,
#                                                                       s=s_opt,
#                                                                       t=t_opt,
#                                                                       x=x_opt,
...
#                                                                       ec=ec_opt_cum,
#                                                                       pwr=pwr_comps)
        opt_mintime_traj.src.result_plots_mintime_compare.result_plots_mintime_compare(pars1=pars1,
                                                                       reftrack1=reftrackF,
                                                                       s1=sF,
                                                                       t1=tF,
                                                                       x1=xF,
    ...
    return -xF[:-1,3],xF[:-1,0], reftrackF, a_interpF, normvectorsF, -x_opt[:-1, 3], x_opt[:-1, 0], reftrack, a_interp, normvectors
